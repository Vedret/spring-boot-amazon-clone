...
cartApp.controller("cartCtrl",function($scope,$http,$log){
	...
	$scope.addCartItem=function(productId){
		...
		$('#total').html(quantity);
		//success callback function will be called if put method exec. successfully
		$http.put('/cart/add/'+productId).success(function(data){
			//We can use another built in angular service which is $log, it has info method which can log the passed object to a console
			//lets try log methods, but first inject $log service to the angular controller
			$log.info(data);
